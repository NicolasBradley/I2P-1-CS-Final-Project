...
//    bullet->coord.y += 0;
    ...
    */
    ...
    bulletCoord.x = bullet->coord.x;
    bulletCoord.y = bullet->coord.y;
    // Check if the bullet collide with the enemies by simple iterating
    enemyNode * cur = enemyList->next;
//    if ((tile_x >= player->coord.y / TILE_SIZE && tile_y >= player->coord.x / TILE_SIZE) && (tile_y <= (player->coord.x+TILE_SIZE-1)/TILE_SIZE &&  tile_x <= (player->coord.y+TILE_SIZE-1)/TILE_SIZE))
//    {
//        hitPlayer(player, bulletCoord, bullet->damage, 0, 0, 0);
//        return true;
//    }
//        
//    else{
        while(cur != NULL){
            ...
            if ((tile_x >= enemyCoord.y / TILE_SIZE && tile_y >= enemyCoord.x / TILE_SIZE) && (tile_y <= (enemyCoord.x+TILE_SIZE-1)/TILE_SIZE &&  tile_x <= (enemyCoord.y+TILE_SIZE-1)/TILE_SIZE))
            {
                hitEnemy(&cur->enemy, bullet->damage, bullet->angle);
                return true;
            }
            /*
                [TODO Hackathon 3-2]D
                Hit the enemy
